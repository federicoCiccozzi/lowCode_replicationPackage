...
plotBubbles <- function(var1, var2, name1, name2) {
  ...
  filePath <- paste("./output/horizontal_analysis_all/", fileName, "_FONT.pdf", sep="")
  pdf(filePath, width=7, height=7)
  par(mar=c(5, 5, 5, 5))
  par(mfrow=c(1, 1))
  par(las=1)
  ...
  plot <- ggplot(counts, aes(counts[,2], counts[,1], color=freq)) + geom_point(aes(size = freq)) +
    ...
    scale_size_continuous(range=c(0, 20)) + geom_text(aes(label = freq), color="black") +
...
plotBubbles_wide <- function(var1, var2, name1, name2) {
  ...
  fileName <- paste(n1, "_", n2)
  fileName <- str_replace_all(fileName, " ", "")
  filePath <- paste("./output/horizontal_analysis_all/", fileName, "_CUT.pdf", sep="")
