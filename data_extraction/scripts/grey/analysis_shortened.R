...
data <- read.delim("./input/data_extraction_form_grey.txt", header = TRUE, quote = "\"", dec = ".", fill = TRUE, na.strings = c(""," "))
studies <- c(49)
...
plot <- function(var, fileName, title, labels, width, height, leftMargin) {
  ...
  filePath <- paste("./output/vertical_analysis_grey/", fileName, "_CUT.pdf", sep="")
...
plotBubbles <- function(var1, var2, name1, name2) {
  ...
  filePath <- paste("./output/horizontal_analysis_grey/", fileName, ".pdf", sep="")
...
### Foundations
labels <- c("MDE", "Component_based", "Visual_programming", "Cloud", "API")
vars5 <- data %>% 
...
### Benefits
labels <- c("GUI_based_development", "Abstraction", "Automation", "Flexibility", "Interoperability", "Improved_RoI",	"Openness")
vars8 <- data %>% 
