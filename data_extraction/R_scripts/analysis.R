...
data <- read.delim("./input/data_extraction_form.txt", header = TRUE, quote = "\"", dec = ".", fill = TRUE, na.strings = c(""," "))
studies <- c(26)
...
plot <- function(var, fileName, title, labels, width, height, leftMargin) {
  ...
  plot <- barplot(dataToPlot, col="#69b3a2", cex.main=1.8, xlim=c(0, studies+10), cex=1.5, cex.names=1.5, las=1, horiz=TRUE)
...
plot(vars0$var0, "publication_type", "Publication type", labels, 10, 5, 13)
...
plot(vars1$var1, "research_type", "Research type", labels, 10, 5, 13)
...
plot(vars2$var2, "questions_type", "Questions type", labels, 10, 5, 13)
...
plot(vars3$var3, "results_type", "Results type", labels, 10, 5, 13)
...
plot(vars4$var4, "validation_type", "Validation type", labels, 10, 5, 13)

### Foundations
labels <- c("Declarative_programming", "GUI", "MDE", "Component_based", "Rapid_app_development", "Code_generation", "Visual_programming", "ABAC", "Access_control_scheme", "Databases", "XACML", "PaaS", "Self_service_functionality", "CASE", "DSL", "Data_models", "Web_technologies", "DevOps", "SAP", "BPMN", "Cloud", "Documentation_generation")
vars5 <- data %>% 
  gather("var5", "value", labels, na.rm = TRUE, convert = TRUE) %>% 
  #dplyr::filter(grepl("x",value)) %>% dplyr::rename(ID = X) %>% 
  select(ID, var5) 
vars5$var5 <- as.factor(vars5$var5)
plot(vars5$var5, "foundations", "Foundations", labels, 15, 15, 13)

### Domains
labels <- c("Education", "Industrial_training", "Web", "Mobile", "Business_process", "Manufacturing", "Enterprise_services", "IoT", "Recommender_systems", "Databases", "Process", "DSL_engineering", "Request_handling", "AI", "Healthcare", "Automotive", "Aeronautics", "Social_media", "Blockchain")
vars6 <- data %>% 
  gather("var6", "value", labels, na.rm = TRUE, convert = TRUE) %>% 
  #dplyr::filter(grepl("x",value)) %>% dplyr::rename(ID = X) %>% 
  select(ID, var6) 
vars6$var6 <- as.factor(vars6$var6)
plot(vars6$var6, "domains", "Domains", labels, 15, 15, 13)

### Tools
labels <- c("Appian", "Kony", "Mendix", "MS_PowerApps", "OutSystems", "Salesforce", "Aurea", "RESTsec", "NETIoT", "Node_RED", "AtmosphericIoT", "Simplifier", "Siemens_MindSphere", "PTCThingWork", "IDM_Cloud", "MS_Azure", "ADAMOS", "INTELLIT", "Zoho_creator", "Google_App_Maker", "App_Cloud", "ZappDev", "Nintex_Workflow_Cloud", "MIT_App_Inventor", "Sysdev_Kalipso", "Zoho_Creator", "KissFlow", "HiCuMES", "Camunda", "AutoML", "AgilePoint", "Sagitec_S3", "Gherkin2OSA", "Lightening", "Temenos_Quantum")
vars7 <- data %>% 
  gather("var7", "value", labels, na.rm = TRUE, convert = TRUE) %>% 
  #dplyr::filter(grepl("x",value)) %>% dplyr::rename(ID = X) %>% 
  select(ID, var7) 
vars7$var7 <- as.factor(vars7$var7)
plot(vars7$var7, "tools", "Tools", labels, 15, 15, 13)

### Benefits
labels <- c("GUI_based_development", "Limited_IT_resources", "Digitalization_BP", "Abstraction", "Automation", "Flexibility", "Customisability", "Usability", "Maintainability", "Interoperability", "Privacy_or_security", "Collaboration_different_stakeholders", "Improved_RoI", "Platform_independence",	"Reusability")
vars8 <- data %>% 
  gather("var8", "value", labels, na.rm = TRUE, convert = TRUE) %>% 
  #dplyr::filter(grepl("x",value)) %>% dplyr::rename(ID = X) %>% 
  select(ID, var8) 
vars8$var8 <- as.factor(vars8$var8)
plot(vars8$var8, "benefits", "Benefits", labels, 15, 15, 13)
